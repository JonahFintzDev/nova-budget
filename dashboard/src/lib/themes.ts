...
export const DEFAULT_THEME_ID = 'budgetscape';
export const DEFAULT_DARK_THEME_ID = 'deep-space';
export const DEFAULT_LIGHT_THEME_ID = 'budgetscape';
...
export const settingsPatchForAppearance = (
...
export const resolveStoredThemeId = (themeId: string): string => {
  return themeId === 'taskscape' ? 'budgetscape' : themeId;
};
...
export const migrateLegacyThemeLocalStorage = (): void => {
  for (const key of ['theme', 'darkTheme', 'lightTheme'] as const) {
    const value = localStorage.getItem(key);
    if (value === 'taskscape') {
      localStorage.setItem(key, 'budgetscape');
    }
  }
};
...
export const stopWatchAutoTheme = (): void => {
